...
class Net(nn.Module):
  ...
  def __init__(self, layersizes, acts):
    ...
    self.acts = acts
    self.syndrome_input = nn.Linear(layersizes[0], layersizes[1])
    self.hidden = [nn.Linear(layersizes[j + 1], layersizes[j + 2]) for j in range(len(acts) - 2)]
    self.error_dist = nn.Linear(layersizes[-2], layersizes[-1])
...
def train(QuantumDecoderNet, *args, **kwargs):
  ...
  for epoch in range(kwargs['epochs']):
    ...
    print("Epoch {}: Loss = {}".format(epoch+1, round(float(loss_epoch), kwargs['precision'])), end = ', ')
    print("Training (Code) = {}, Validation (Code) = {}".format(round(train_acc_codespace_epoch, kwargs['precision']), round(valid_acc_codespace_epoch, kwargs['precision'])), end = ', ')
    print("Training (X) = {}, Validation (X) = {}".format(round(train_acc_x_epoch, kwargs['precision']), round(valid_acc_x_epoch, kwargs['precision'])), end = ', ')
    print("Training (Z) = {}, Validation (Z) = {}".format(round(train_acc_z_epoch, kwargs['precision']), round(valid_acc_z_epoch, kwargs['precision'])))
...
def accuracy(QuantumDecoderNet, ds_synds, ds_error_labels, **kwargs):
  ...
  with torch.no_grad():
    for idx in range(l):
      output = QuantumDecoderNet.forward(ds_synds[idx]).detach().numpy()
      for _ in range(kwargs['num_random_trials']):
        ...
        actual_syndrome = ds_synds[idx].detach().numpy()
        if np.array_equal(predicted_syndrome, actual_syndrome):
          num_success += 1
          corrected = np.array([int(b[i]) ^ int(ds_error_labels[idx].detach().numpy()[i]) for i in range(len(b))])
